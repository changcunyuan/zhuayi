...
class image extends http
{
	...
	function show($file,$referer = '')
	{
		...
		if (is_array($file))
		{
			if ($file['error'] > 0)
			{
				throw new Exception("上传图片失败,可能图片大小超出限制,只允许上传".intval(($this->max_length/1024/1024))."M以内的图片", -1);
			}
			if ($this->max_length < $file['size'])
			{
				throw new Exception("图片大小超出限制,只允许上传".intval(($this->max_length/1024/1024))."M以内的图片", -1);
			}
		...
		{
			throw new Exception("图片格式不正确，请您输入.png、.jpg、.jpeg格式的图片！", -1);
		}
	...
	function save_temp($image_p,$type)
	{
		...
		{ 
			case 1:
			imagegif($image_p, $this->filename,100);
			break; 
			case 2:
			imagejpeg($image_p, $this->filename,100);
			break; 
			case 3:
	 ...
	 * @author zhuayi
	 */
	function save($filename,$cdn = true)
	{
		...
		$put_array['filename'] = trim($filename);
		$put_array['filedata'] = $this->file_data;
		$put_array['key'] = $this->cdn_key;

		if ($cdn === true)
		{
			/* 写入文件 */
			$this->put($this->cdn_server,$put_array);
			...
			{
				throw new Exception($reset['msg'], -1);
			}
		}
		else
		{
			
			$reset = $this->file->write($_SERVER['SAE_FILE_PATH'].$filename,$this->file_data);
